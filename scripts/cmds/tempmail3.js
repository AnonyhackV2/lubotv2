...
module.exports = {
  config: {
    name: "tempmail3",
    aliases: ["temp3","email3","gmail3"], 
    version: "1.0",
    author: "modified by Kyle",
    countDown: 5,
    role: 0,
    category: "tool",
  },
  ...
  onStart: async function ({ api, args, event }) {
    try {
      if (args.length === 0) {
        return api.sendMessage("💁🏻‍♂️ 𝗭𝗘𝗣𝗛𝗬𝗥𝗨𝗦 𝗡𝗢𝗧𝗜𝗙.\n▬▬▬▬▬▬▬▬▬▬▬▬\n\nℹ️ Use 'tempmail3 gen' to generate a temporary email or 'tempmail3 inbox (email)' to retrieve inbox messages.", event.threadID, event.messageID);
      }
      ...
      if (command === "gen") {
        ...
        return api.sendMessage(`❌ | 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗰𝗼𝗺𝗺𝗮𝗻𝗱.\n▬▬▬▬▬▬▬▬▬▬▬▬\n💁🏻‍♂️ Use 'tempmail3 gen' to generate a temporary email or 'tempmail3 inbox (email)' to retrieve inbox messages.`, event.threadID, event.messageID);
      }
    } catch (error) {
      console.error(error);
